...
{#if !txType}
	<EmptyState icon="lucide:search-x" title="Transaction not found" description="This transaction may have been deleted or hasn't synced yet.">
			...
			<div class="lg:col-span-2 space-y-4">
				...
				<div class="surface-card accent-bar p-6" style="--accent: var(--ui-color-primary-500);">
					<div class="flex items-center gap-4">
						...
						<div>
							<div class="text-[12px] font-semibold text-[var(--ui-text-muted)]">Amount</div>
				...
				<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
					...
					<div class="divide-y divide-[var(--ui-border-muted)] text-[13px]">
						...
						<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:type" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Type</span><span class="ml-auto font-semibold capitalize">{txType}</span></div>
						<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:circle-dot" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Status</span><span class="ml-auto"><Badge color={statusColor(txStatus)}>{txStatus}</Badge></span></div>
						<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:clock" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Date</span><span class="ml-auto font-semibold">{txDate ? relativeTime(txDate as string) : '—'}</span></div>
						...
						{#if txType === 'payment' && payment?.data.method}<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:wallet" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Method</span><span class="ml-auto font-semibold capitalize">{(payment.data as any).method}</span></div>{/if}
						{#if txType === 'order' && order?.data.customerName}<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:user" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Customer</span><span class="ml-auto font-semibold">{order.data.customerName}</span></div>{/if}
					</div>
				</div>
				...
				{#if txType === 'order' && order?.data.lines?.length}
					<div class="surface-card">
						<div class="px-5 py-3"><h2 class="font-display text-[14px] font-semibold">Items</h2></div>
						<table class="w-full text-left text-[13px]">
							<thead><tr><th class="px-5 py-2.5">Item</th><th class="px-5 py-2.5 text-center">Qty</th><th class="px-5 py-2.5 text-right">Total</th></tr></thead>
			...
			<div class="space-y-4">
				...
				{#if txType === 'order' && order}
					<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
						...
						<div class="space-y-2 px-5 py-4 text-[13px]">
							...
							{#if order.data.taxAmount}<div class="flex justify-between"><span class="text-[var(--ui-text-muted)]">Tax</span><span class="tabular-nums">{formatMoney((order.data as any).taxAmount ?? 0, currency)}</span></div>{/if}
							{#if order.data.discount}<div class="flex justify-between"><span class="text-[var(--ui-text-muted)]">Discount</span><span class="tabular-nums text-[var(--tone-error-text)]">−{formatMoney((order.data as any).discount ?? 0, currency)}</span></div>{/if}
							<div class="flex justify-between border-t border-[var(--ui-border-muted)] pt-2 font-display text-[15px] font-bold"><span>Total</span><span class="tabular-nums">{formatMoney((order.data as any).total ?? 0, currency)}</span></div>
						</div>
					</div>
				{/if}
				...
				{#if txType === 'payment' && payment}
					<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
						...
						<div class="divide-y divide-[var(--ui-border-muted)] text-[13px]">
							{#if payment.data.orderId}<div class="flex items-center gap-3 px-5 py-3"><Icon name="lucide:link" class="size-4 text-[var(--ui-text-dimmed)]" /><span class="text-[var(--ui-text-muted)]">Order</span><a href="/transactions/{payment.data.orderId}" class="ml-auto font-mono text-[12px] text-primary-500">{payment.data.orderId.slice(0, 12)}</a></div>{/if}
