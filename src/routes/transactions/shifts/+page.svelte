	...
	async function openShift() {
		await glo.upsert<Shift>(
			TYPE.shift,
			{
				...
				status: 'active',
				openingCash: typeof opening === 'number' ? opening : Number(opening) || 0,
				currency,
	...
	async function closeShift() {
		...
		await glo.upsert<Shift>(
			TYPE.shift,
			{
				...activeShift.data,
				status: 'closed',
				closingCash: activeShift.data.openingCash,
				expectedCash: activeShift.data.openingCash,
				difference: 0,
	...
	async function addCashEvent() {
		...
		await glo.upsert<CashEvent>(
			TYPE.cashEvent,
			{
				...
				type: cashType,
				amount: typeof cashAmt === 'number' ? cashAmt : Number(cashAmt) || 0,
				currency,
				reason: cashReason.trim() || undefined,
				occurredAt: new Date().toISOString()
			},
			{ id: newRecordId('cash-event') }
		);
		toast.success('Cash event recorded');
		cashAmt = '';
		cashReason = '';
	}
</script>
...
<div class="space-y-4">
	<div>
		...
		<p class="text-[12.5px] text-[var(--ui-text-muted)]">
			Open/close shifts, record cash movements · kinds 30520/30521
		</p>
	</div>
	...
	{#if activeShift}
		<div
			class="accent-bar surface-card flex flex-wrap items-center justify-between gap-3 p-5"
			style="--accent:var(--tone-success-text);"
		>
			...
			<Button color="neutral" variant="subtle" icon="lucide:lock" onclick={closeShift}
				>Close shift</Button
			>
		</div>
	{:else}
	...
	<div class="surface-card flex flex-wrap items-end gap-2 p-4">
		...
		>
			<select
				...
				><option value="cash_in">Cash in</option><option value="cash_out">Cash out</option><option
					value="paid_out">Paid out</option
				><option value="bank_deposit">Bank deposit</option></select
			><Icon
	...
	{#if events.length === 0}
		<EmptyState
			...
			description="Cash in/out, paid outs and bank deposits appear here."
		/>
	{:else}
		<div class="data-panel">
			<table class="table-surface w-full text-left">
				<thead
					><tr
						><th class="px-5 py-2.5">Type</th><th class="px-5 py-2.5">Reason</th><th
							class="px-5 py-2.5 text-right">Amount</th
						><th class="px-5 py-2.5 text-right">When</th></tr
					></thead
				>
				<tbody class="divide-y divide-[var(--ui-border-muted)] text-[13px]">
					{#each events.slice(0, 50) as e (e.id)}<tr
							><td class="px-5 py-3"
								><Badge color={e.data.type === 'cash_in' ? 'success' : 'info'}
									>{e.data.type.replace('_', ' ')}</Badge
								></td
							><td class="px-5 py-3 text-[var(--ui-text-muted)]">{e.data.reason ?? '—'}</td><td
	...
	{#if shifts.length}
		<div class="data-panel">
			...
			<table class="table-surface w-full text-left">
				<thead
					><tr
						><th class="px-5 py-2.5">Shift</th><th class="px-5 py-2.5">Status</th><th
							class="px-5 py-2.5 text-right">Opening</th
						><th class="px-5 py-2.5 text-right">Closed</th></tr
					></thead
				>
				<tbody class="divide-y divide-[var(--ui-border-muted)] text-[13px]">
					{#each shifts as s (s.id)}<tr
							><td class="px-5 py-3 font-mono text-[12.5px]">{s.data.number}</td><td
								class="px-5 py-3"
								><Badge color={statusColor(s.data.status)}>{s.data.status}</Badge></td
							><td class="px-5 py-3 text-right tabular-nums"
								>{formatMoney(s.data.openingCash, s.data.currency || currency)}</td
							><td class="px-5 py-3 text-right text-[12px] text-[var(--ui-text-dimmed)]"
								>{s.data.closedAt ? relativeTime(s.data.closedAt) : '—'}</td
							></tr
						>{/each}
				</tbody>
			</table>
