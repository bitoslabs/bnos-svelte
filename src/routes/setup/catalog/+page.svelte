<script lang="ts">
	...
	async function seedSamples() {
		for (const data of SAMPLE) await glo.upsert<GloProduct>('catalog.product', data);
		count = glo.all<GloProduct, 'catalog.product'>('catalog.product').length;
	}
	function clear() {
...
<div class="space-y-5">
	<div>
		...
		<p class="mt-1 text-[13.5px] text-[var(--ui-text-muted)]">
			Products are GLO <code>catalog.product</code> objects (kind 30100). Add a few samples to start, then
			manage everything from Catalog.
		</p>
	</div>

	<div class="surface-card flex items-center justify-between p-4">
		<div class="flex items-center gap-3">
			<div class="grid size-10 place-items-center rounded-lg bg-primary-500/10 text-primary-600 dark:text-primary-400">
				<Icon name="lucide:package" class="size-5" />
			</div>
			<div>
				<div class="text-[13px] font-semibold">{count} product{count === 1 ? '' : 's'}</div>
				<div class="text-[12px] text-[var(--ui-text-muted)]">in your local catalog</div>
			</div>
		</div>
		<div class="flex gap-2">
			<Button color="neutral" variant="ghost" size="sm" onclick={clear} disabled={count === 0}>Clear</Button>
			<Button color="primary" size="sm" icon="lucide:sparkles" onclick={seedSamples}>Add samples</Button>
		</div>
	</div>
			...
			{#each glo.all<GloProduct, 'catalog.product'>('catalog.product') as p (p.id)}
				<li class="flex items-center justify-between px-4 py-3">
					...
					<span class="font-mono text-[12.5px] text-[var(--ui-text-muted)]">
						{(p.data.currency ?? '')} {(p.data.price ?? 0).toLocaleString()}
					</span>
				</li>
			{/each}
		</ul>
