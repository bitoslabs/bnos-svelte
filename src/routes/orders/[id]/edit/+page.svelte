<script lang="ts">
	...
	function removeLine(lineId: string) { lines = lines.filter((l) => l.id !== lineId); }
	function incQty(lineId: string) { lines = lines.map((l) => l.id === lineId ? { ...l, quantity: l.quantity + 1 } : l); }
	function decQty(lineId: string) { lines = lines.map((l) => l.id === lineId ? { ...l, quantity: Math.max(1, l.quantity - 1) } : l); }
	...
	const changes = $derived(() => {
		...
		if (orderType !== (d.type ?? 'takeaway')) list.push({ label: 'Type', oldVal: d.type, newVal: orderType });
		if (total !== (d.total ?? 0)) list.push({ label: 'Total', oldVal: formatMoney(d.total ?? 0, currency), newVal: formatMoney(total, currency) });
		if (customerName !== (d.customerName ?? '')) list.push({ label: 'Customer', oldVal: d.customerName ?? '—', newVal: customerName || '—' });
		if (notes !== (d.notes ?? '')) list.push({ label: 'Notes', oldVal: d.notes ?? '', newVal: notes });
		if (priority !== (d.priority ?? 'normal')) list.push({ label: 'Priority', oldVal: d.priority ?? 'normal', newVal: priority });
		if (lines.length !== (d.lines?.length ?? 0)) list.push({ label: 'Items', oldVal: d.lines?.length ?? 0, newVal: lines.length });
		return list;
	});
	...
	<div class="space-y-4">
		...
		<div class="flex flex-wrap items-start justify-between gap-3">
			<div class="flex items-center gap-3">
				<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:arrow-left" onclick={() => goto(`/orders/${id}`)} />
				<div>
					<div class="flex items-center gap-2">
						<h1 class="font-display text-xl font-bold tracking-tight">Edit {order.data.orderNumber ?? ('#' + (id ?? '').slice(0, 8))}</h1>
						<Badge color="neutral">{titleCase(order.data.status)}</Badge>
					</div>
					<p class="text-[12px] text-[var(--ui-text-muted)]">Modify items, quantities, and order details</p>
				</div>
			</div>
			<Button color="neutral" variant="subtle" size="sm" icon="lucide:eye" href="/orders/{id}">View</Button>
		</div>

		<!-- Change tracking summary -->
		{#if loaded && changes().length > 0}
			<div class="flex items-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-2.5">
				...
				<span class="text-[12.5px] font-medium text-amber-700 dark:text-amber-300">
					{changes().length} change{changes().length === 1 ? '' : 's'}: {changes().map((c) => c.label).join(', ')}
				</span>
			</div>
		{/if}
		...
		<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
			...
			<div class="space-y-4 lg:col-span-2">
				...
				<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
					...
					<div class="px-5 py-4 space-y-3">
						<!-- Barcode -->
						<div class="flex items-center gap-2">
							<div class="inline-flex flex-1 items-center gap-2 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-bg-muted)] px-3 py-2 transition-colors focus-within:border-primary-500">
				...
				<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
					...
					<div class="px-5 py-4 space-y-3">
						<label class="block">
							...
							<Input bind:value={notes} textarea={true} rows={2} placeholder="Special instructions…" class="w-full" />
						</label>
						<label class="block">
			...
			<div class="space-y-4">
				...
				<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
					...
					<div class="px-5 py-4">
						<span class="mb-2 block text-[11px] font-bold uppercase tracking-wider text-[var(--ui-text-muted)]">Order Status</span>
				...
				<div class="space-y-2">
					...
					<Button color="neutral" variant="ghost" block onclick={() => goto(`/orders/${id}`)}>Cancel</Button>
				</div>
			</div>
		</div>
	</div>
