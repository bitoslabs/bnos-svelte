<script lang="ts">
	...
	onMount(() => {
		if (browser) {
			try {
				...
				if (r) enabled = { ...enabled, ...JSON.parse(r) };
			} catch {
				/* */
			}
		}
	...
	function toggle(k: string, v: boolean) {
		enabled[k] = v;
		if (browser) localStorage.setItem(KEY, JSON.stringify(enabled));
		toast.success(`${METHODS.find((m) => m.key === k)?.label} ${v ? 'enabled' : 'disabled'}`);
	}
</script>
	...
	<div class="surface-card divide-y divide-[var(--ui-border-muted)]">
		{#each METHODS as m (m.key)}
			<div class="flex items-center gap-3 px-4 py-3.5">
				<div
					class="grid size-10 shrink-0 place-items-center rounded-xl bg-[var(--ui-bg-accented)] text-[var(--ui-text-muted)]"
				>
					<Icon name={m.icon} class="size-5" />
				</div>
				<div class="flex-1">
					<div class="font-semibold">{m.label}</div>
					<div class="text-[12px] text-[var(--ui-text-muted)]">{m.desc}</div>
				</div>
