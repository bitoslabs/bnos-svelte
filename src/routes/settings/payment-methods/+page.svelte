<script lang="ts">
	...
	const BUILTINS: PaymentMethod[] = [
		{ id: 'cash', label: 'Cash', icon: 'lucide:banknote', enabled: true, type: 'cash', order: 0, builtin: true },
		{ id: 'card', label: 'Card', icon: 'lucide:credit-card', enabled: true, type: 'card', order: 1, builtin: true },
		{ id: 'qr', label: 'QR / Bank transfer', icon: 'lucide:qr-code', enabled: false, type: 'qr', order: 2, builtin: true },
		{ id: 'lightning', label: 'Lightning', icon: 'lucide:zap', enabled: false, type: 'lightning', order: 3, builtin: true }
	];
...
<div class="space-y-4">
	...
	<div class="flex items-center justify-between gap-3">
		<div class="flex items-center gap-3">
			<Icon name="lucide:credit-card" class="size-5 text-primary-500" />
			<div>
				<h2 class="font-display text-[15px] font-semibold tracking-tight">Payment methods</h2>
				<p class="text-[12px] text-[var(--ui-text-muted)]">Methods available at checkout</p>
			</div>
		</div>
		<div class="flex items-center gap-2">
			<Button variant="ghost" color="neutral" size="sm" icon="lucide:rotate-ccw" onclick={handleReset}>
				Reset
			</Button>
	...
	{#if loaded && sortedMethods.length === 0}
		<EmptyState icon="lucide:credit-card" title="No payment methods" description="Add your first payment method to start accepting payments.">
			{#snippet actions()}
				<Button variant="solid" color="primary" size="sm" icon="lucide:plus" onclick={openAdd}>
					Add method
				</Button>
		...
		<div class="space-y-2">
			{#each sortedMethods as m (m.id)}
				<div
					...
					class="surface-card flex items-center gap-3 px-4 py-3.5 transition-opacity {!m.enabled ? 'opacity-50' : ''}"
				>
					<!-- Icon -->
					<div
						class="grid size-10 shrink-0 place-items-center rounded-xl {TYPE_STYLES[m.type] ?? TYPE_STYLES.other}"
					>
					...
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-2">
							<span class="font-semibold text-[14px]">{m.label}</span>
...
<Dialog bind:open={showDialog} title={editing ? 'Edit payment method' : 'Add payment method'}>
	<div class="space-y-4">
		...
		<div class="space-y-1.5">
			<label class="block text-[11px] font-bold uppercase tracking-wider text-[var(--ui-text-muted)]">
				Label
			</label>
		...
		<div class="space-y-1.5">
			...
			<div class="flex flex-wrap gap-2">
				{#each ICON_OPTIONS as opt (opt.value)}
					<button
						...
						title={opt.label}
						class="grid size-10 place-items-center rounded-xl border transition {formIcon === opt.value
			...
			<div class="flex items-center gap-2 pt-1">
				...
				<div
					class="grid size-8 place-items-center rounded-lg {TYPE_STYLES[formType] ?? TYPE_STYLES.other}"
				>
			...
			<div class="flex flex-wrap gap-1.5">
				{#each TYPE_OPTIONS as opt (opt.value)}
					<button
						...
						class="rounded-lg border px-2.5 py-1.5 text-[12px] font-medium transition {formType === opt.value
							? 'border-primary-500 bg-primary-500/10 text-primary-600 dark:text-primary-400'
