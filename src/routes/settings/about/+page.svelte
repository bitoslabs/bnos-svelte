<script lang="ts">
	...
	const buildRows = [
		...
		{ label: 'Framework', value: 'SvelteKit · Svelte 5', sub: 'Tailwind v4', icon: 'lucide:component' },
		{ label: 'Identity', value: 'Nostr · NIP-07 / nsec', icon: 'lucide:fingerprint' },
		{ label: 'Storage', value: 'Local-first', sub: 'localStorage + Nostr relays', icon: 'lucide:database' }
	];

	const legalLinks = [
		{ to: '/legal/privacy', icon: 'lucide:shield-check', label: 'Privacy Policy', desc: 'What we store and how' },
		{ to: '/legal/terms', icon: 'lucide:file-text', label: 'Terms of Service', desc: 'Using this software' },
		{ to: '/legal/license', icon: 'lucide:scale', label: 'Open-source License', desc: 'MIT · your permissions' }
	] as const;
...
<div class="space-y-5">
	<PageHeader
		icon="lucide:info"
		title="About"
		description="Version, open-source & data layer"
	/>
	...
	<section class="surface-card overflow-hidden">
		<div class="relative px-6 pb-6 pt-8 text-center sm:px-8">
			...
			<div class="pointer-events-none absolute inset-0 overflow-hidden">
				<div class="absolute -top-24 left-1/2 size-56 -translate-x-1/2 rounded-full bg-primary-500/10 blur-3xl"></div>
			</div>
			...
			<div class="relative">
				...
				{#if session.npub}
					...
					>
						...
						Signed in as <span class="font-mono font-semibold">{truncateNpub(session.npub, 10, 6)}</span>
					</div>
				{/if}
			</div>
		</div>
	...
	<section class="surface-card divide-y divide-[var(--ui-border-muted)]">
		<div class="flex items-center gap-2 px-5 py-3">
			...
			<h2 class="font-display text-[14px] font-semibold">Version &amp; build</h2>
		</div>
		<div>
	...
	<section class="surface-card">
		<div class="flex items-center gap-2 px-5 py-3">
			...
			<h2 class="font-display text-[14px] font-semibold">Links &amp; open source</h2>
			<Badge color="success" class="ml-auto"><Icon name="lucide:heart" class="mr-1 size-3" />Open source</Badge>
		</div>
					...
					<span class="min-w-0 flex-1">
						<span class="flex items-center gap-1 text-[13px] font-semibold text-[var(--ui-text)]">
							...
							<Icon
								...
								class="size-3.5 text-[var(--ui-text-dimmed)] transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
							/>
						</span>
						<span class="mt-0.5 block text-[11.5px] text-[var(--ui-text-dimmed)]">{link.description}</span>
					</span>
				</a>
			{/each}
		</div>
		<div class="flex flex-wrap items-center justify-between gap-2 border-t border-[var(--ui-border-muted)] px-5 py-3">
			<a
	...
	<section class="surface-card">
		<div class="flex items-center gap-2 px-5 py-3">
			...
			<h2 class="font-display text-[14px] font-semibold">Legal</h2>
		</div>
		<div class="divide-y divide-[var(--ui-border-muted)]">
			{#each legalLinks as l (l.to)}
				...
				>
					<Icon name={l.icon} class="size-4 shrink-0 text-[var(--ui-text-dimmed)] group-hover:text-primary-500" />
					<span class="min-w-0 flex-1">
	...
	<section class="surface-card flex flex-wrap items-center justify-between gap-3 p-5">
		<div class="min-w-0">
			<h2 class="font-display text-[14px] font-semibold">Manage your account</h2>
			<p class="mt-0.5 text-[12px] text-[var(--ui-text-muted)]">
				Sign in with a different identity or generate a new Nostr account.
			</p>
		</div>
		<div class="flex flex-wrap gap-2">
			<Button
				...
				variant="subtle"
				size="sm"
				icon="lucide:log-in">Sign in</Button
			>
			<Button
				href={resolve('/login')}
				color="primary"
				variant="soft"
				size="sm"
				icon="lucide:user-plus">Create account</Button
			>
		</div>
	</section>
	...
	<p class="px-2 pb-1 text-center text-[11px] leading-relaxed text-[var(--ui-text-dimmed)]">
		© {new Date().getFullYear()} {site.name} contributors · {site.license} licensed
		<br />
		...
		<a
			...
			target="_blank"
			rel="noopener"
			class="font-semibold text-[var(--ui-text-muted)] hover:text-primary-500">{site.sponsor.name}</a
		>.
	</p>
</div>
