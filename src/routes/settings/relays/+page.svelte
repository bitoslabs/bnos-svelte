	...
	<div class="flex items-center gap-3">
		...
		<div>
			...
			<p class="text-[12px] text-[var(--ui-text-muted)]">
				{relays.relays.length} configured · {relays.readableRelays.length} read · {relays
					.writableRelays.length} write · {relays.online ? 'online' : 'offline'}
			</p>
		</div>
	</div>
			...
			<li class="flex items-center gap-3 px-4 py-3">
				...
				<div class="flex items-center gap-3">
					<div class="flex items-center gap-1.5">
						...
						<Switch
							checked={relays.canRead(url)}
							onCheckedChange={(enabled) => relays.setPermission(url, 'read', enabled)}
						/>
					</div>
					<div class="flex items-center gap-1.5">
						<Badge color={relays.canWrite(url) ? 'primary' : 'neutral'}>W</Badge>
						<Switch
							checked={relays.canWrite(url)}
							onCheckedChange={(enabled) => relays.setPermission(url, 'write', enabled)}
						/>
					</div>
				</div>
				<Button
