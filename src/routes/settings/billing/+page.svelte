<script lang="ts">
	...
	const PLANS: Record<PlanCode, PlanInfo> = {
		lite: {
			...
			modules: { restaurant: true, inventory: true, multiBranch: false, purchaseOrders: false, reports: true }
		},
		starter: {
			code: 'starter',
			name: 'Solo',
			...
			modules: { restaurant: true, inventory: true, multiBranch: true, purchaseOrders: false, reports: true }
		},
		pro: {
			code: 'pro',
			name: 'Business',
			...
			limits: { ordersPerDay: 2000, branches: 5, staff: 25, products: 20000, customers: 'unlimited' },
			modules: { restaurant: true, inventory: true, multiBranch: true, purchaseOrders: true, reports: true }
		},
		pro_max: {
			code: 'pro_max',
			name: 'Premium',
			...
			limits: { ordersPerDay: 'unlimited', branches: 15, staff: 100, products: 'unlimited', customers: 'unlimited' },
			modules: { restaurant: true, inventory: true, multiBranch: true, purchaseOrders: true, reports: true }
		},
		enterprise: {
			code: 'enterprise',
			...
			limits: { ordersPerDay: 'unlimited', branches: 'unlimited', staff: 'unlimited', products: 'unlimited', customers: 'unlimited' },
			modules: { restaurant: true, inventory: true, multiBranch: true, purchaseOrders: true, reports: true }
		}
	};
	...
	type SubStatus = 'trialing' | 'active' | 'past_due' | 'grace' | 'cancel_scheduled' | 'cancelled' | 'expired';
	...
	const isActive = $derived(subscription?.status === 'active' || subscription?.status === 'trialing');
	...
	function processPayment() {
		...
		if (selectedMethod === 'btc_lightning') {
			...
			activeLightningInvoice = 'lnbc' + inv.amount + '0n1ps' + Math.random().toString(36).slice(2, 20);
	...
	const paymentMethodOptions = [
		{ code: 'btc_lightning', label: 'Bitcoin Lightning', icon: 'lucide:zap', iconColor: 'text-amber-500', bgColor: 'bg-amber-500/10' },
		{ code: 'phajay_qr', label: 'Bank QR', icon: 'lucide:qr-code', iconColor: 'text-primary-500', bgColor: 'bg-primary-500/10' }
	];
...
<div class="space-y-5">
	...
	<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
		<div>
			...
			<p class="text-[12.5px] text-[var(--ui-text-muted)]">Manage your subscription, plan, and payment history</p>
		</div>
		{#if subscription}
			<span class="inline-flex items-center gap-1.5 self-start rounded-full border px-3 py-1 text-xs font-bold {STATUS_COLORS[subscription.status] ?? STATUS_COLORS.expired}">
	...
	{#if !subscription}
		...
		<div class="space-y-6">
			<!-- Hero CTA -->
			<div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-600 to-indigo-700 p-6 text-white shadow-lg sm:p-8 dark:from-primary-800 dark:to-indigo-950">
				...
				<div class="relative max-w-2xl">
					<span class="mb-4 inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 text-xs font-bold tracking-wider uppercase backdrop-blur-sm">
					...
					<p class="mt-2 max-w-xl text-sm leading-relaxed text-white/75">
						Unlock multi-branch, advanced reports, and higher limits. No credit card required — try any plan free for 30 days.
					</p>
					<div class="mt-5">
						<Button color="neutral" variant="solid" size="lg" icon="lucide:play" onclick={startTrial}>Start free trial</Button>
					</div>
				</div>
			</div>
						...
						<div class="surface-card flex flex-col justify-between p-4">
							<div>
								<div class="mb-2 flex items-center justify-between">
									<span class="rounded-md bg-primary-500/10 px-2 py-0.5 text-xs font-bold tracking-wide uppercase text-primary-500">
										{plan.name}
									</span>
									<Icon name={plan.icon} class="size-4 text-[var(--ui-text-dimmed)]" />
								</div>
								<p class="text-lg font-black tracking-tight">{formatPlanPrice(code)}</p>
								<p class="mt-1 text-[11.5px] leading-relaxed text-[var(--ui-text-muted)]">{plan.description}</p>
							</div>
							<div class="mt-4 space-y-1.5 border-t border-[var(--ui-border-muted)] pt-3 text-[11.5px] text-[var(--ui-text-muted)]">
								<div class="flex items-center gap-1.5">
									<Icon name="lucide:check-circle" class="size-3.5 shrink-0 text-emerald-500" />
									Orders/day: <strong class="text-[var(--ui-text)]">{getPlanLimitText(code, 'ordersPerDay')}</strong>
								</div>
								<div class="flex items-center gap-1.5">
									<Icon name="lucide:check-circle" class="size-3.5 shrink-0 text-emerald-500" />
									Branches: <strong class="text-[var(--ui-text)]">{getPlanLimitText(code, 'branches')}</strong>
								</div>
								<div class="flex items-center gap-1.5">
									<Icon name="lucide:check-circle" class="size-3.5 shrink-0 text-emerald-500" />
									Products: <strong class="text-[var(--ui-text)]">{getPlanLimitText(code, 'products')}</strong>
								</div>
							</div>
						</div>
					{/each}
	...
	{:else}
		...
		<div class="grid grid-cols-1 gap-5 lg:grid-cols-3 lg:items-start">
			...
			<div class="space-y-5 lg:col-span-2">
				<!-- Membership Card -->
				<div class="relative overflow-hidden rounded-2xl bg-gradient-to-br {currentPlan.gradient} {currentPlan.shadow} p-5 text-white shadow-sm sm:p-6">
					<div class="absolute inset-0 opacity-10">
						<div class="absolute -top-16 -right-16 size-64 rounded-full bg-white/30 blur-3xl"></div>
						<div class="absolute -bottom-16 -left-16 size-48 rounded-full bg-white/20 blur-2xl"></div>
					</div>
					<div class="relative space-y-4">
						<div class="flex items-start justify-between gap-3">
							<div class="min-w-0">
								<span class="mb-2 inline-flex items-center rounded-full bg-white/20 px-2 py-0.5 text-xs font-bold uppercase backdrop-blur-sm">
									{statusLabel}
								</span>
								<h3 class="text-2xl font-black tracking-tight capitalize sm:text-3xl">{currentPlan.name}</h3>
							</div>
							<div class="grid size-10 shrink-0 place-items-center rounded-xl border border-white/20 bg-white/15 backdrop-blur-sm">
								<Icon name={currentPlan.icon} class="size-5" />
							</div>
						</div>
						<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
							<div class="space-y-1.5">
								<div class="inline-flex items-center gap-1.5 rounded-lg border border-white/5 bg-white/10 px-2.5 py-1 text-xs backdrop-blur-sm">
									<Icon name="lucide:lock" class="size-3.5 text-white/60" />
									<span class="truncate font-mono max-w-[140px] sm:max-w-none">{subscription.companyId}</span>
									<button type="button" onclick={() => copyText(subscription!.companyId, 'Company ID')} class="cursor-pointer p-0.5 hover:text-white">
								...
								<div class="flex items-center gap-1 text-xs font-medium text-white/55">
									...
									Support: <span class="font-bold capitalize text-white/80">{planCode === 'lite' ? 'community' : planCode === 'enterprise' ? 'dedicated' : 'email'}</span>
								</div>
							</div>
							<div class="shrink-0 text-left sm:text-right">
								<span class="text-2xl font-black tracking-tight sm:text-3xl">{formatPlanPrice(planCode)}</span>
						...
						<div class="space-y-1.5 border-t border-white/10 pt-3">
							...
							<div class="h-1.5 w-full overflow-hidden rounded-full bg-white/15">
								<div class="h-full rounded-full bg-white/80 transition-all duration-1000" style="width: {periodProgressPercent}%"></div>
							</div>
						</div>
					</div>
				</div>
								...
								<div class="rounded-xl border border-[var(--ui-border-muted)] bg-[var(--ui-bg-muted)] p-4">
									<div class="mb-3 flex items-start justify-between">
										...
										<span class="text-xs font-bold {getThresholdColor(entry.threshold)}">
											{entry.current} / <span class="opacity-70">{entry.limit === 'unlimited' ? '∞' : entry.limit}</span>
										</span>
									</div>
									{#if entry.limit !== 'unlimited'}
										<div class="space-y-1">
											<div class="h-1.5 w-full overflow-hidden rounded-full bg-[var(--ui-bg-accented)]">
												<div class="h-full rounded-full transition-all duration-500 {getThresholdBarColor(entry.threshold)}" style="width: {Math.min(entry.percent, 100)}%"></div>
											</div>
											<div class="text-right text-xs font-bold {getThresholdColor(entry.threshold)}">{entry.percent}%</div>
										</div>
									{:else}
										<div>
											<div class="h-1.5 w-full overflow-hidden rounded-full bg-emerald-500/10">
												<div class="h-full w-full bg-gradient-to-r from-emerald-500/20 to-emerald-400/40"></div>
											</div>
											<div class="mt-1.5 flex items-center justify-between text-xs font-bold text-emerald-500">
												<span>Unlimited</span>
			...
			<div class="space-y-5">
				...
				<section class="surface-card overflow-hidden">
					...
					<div class="space-y-3 p-4">
						{#if isGrace}
							<div class="flex items-start gap-2.5 rounded-xl border border-red-200/40 bg-red-50 p-3 dark:bg-red-950/20">
								...
								<div class="text-xs">
									...
									<p class="mt-0.5 leading-relaxed text-red-700/80 dark:text-red-400/70">
										Your subscription is {statusLabel}. Please resolve payment to restore full access.
									</p>
								</div>
							</div>
						{/if}
						<div class="flex flex-col gap-2">
							{#if canUpgrade}
								<Button color="primary" variant="solid" block icon="lucide:arrow-up" onclick={() => { selectedPlan = null; showUpgradeModal = true; }}>Upgrade</Button>
							{/if}
							{#if canDowngrade}
								<Button color="neutral" variant="subtle" block icon="lucide:arrow-down" onclick={() => { selectedPlan = null; showDowngradeModal = true; }}>Downgrade</Button>
							{/if}
							{#if canCancel}
								<Button color="error" variant="ghost" block size="sm" icon="lucide:trash-2" onclick={() => { showCancelModal = true; }}>Cancel subscription</Button>
							{/if}
						</div>
					</div>
				</section>
				...
				{#if outstandingInvoices.length > 0}
					<section class="overflow-hidden rounded-2xl border border-amber-200/50 bg-gradient-to-br from-amber-50 to-orange-50 shadow-sm dark:border-amber-900/30 dark:from-amber-950/15 dark:to-orange-950/10">
						<div class="flex items-center gap-2 border-b border-amber-200/40 px-5 py-3 dark:border-amber-900/20">
						...
						<div class="space-y-2 p-4">
							{#each outstandingInvoices as inv (inv.id)}
								<div class="flex items-center justify-between rounded-xl border border-amber-200/30 bg-white/80 p-3 dark:border-amber-500/10 dark:bg-black/25">
									...
									<div class="flex flex-col items-end gap-1.5">
										<span class="text-xs font-bold text-orange-600 dark:text-orange-400">{formatAmount(inv.amount)}</span>
										<Button size="sm" color="primary" variant="subtle" icon="lucide:send" onclick={() => openPayModal(inv)}>Pay now</Button>
									</div>
								</div>
							{/each}
						</div>
				...
				{#if paymentHistory.length > 0}
					<section class="surface-card overflow-hidden">
						...
						<div class="p-4 sm:p-5">
							<div class="relative space-y-4 pl-6 before:absolute before:bottom-2 before:left-[11px] before:top-2 before:w-0.5 before:bg-[var(--ui-border-muted)]">
								{#each paymentHistory as p (p.id)}
									<div class="relative flex items-start justify-between gap-3">
										<div class="absolute -left-[29px] top-0.5 z-10 grid size-6 place-items-center rounded-full border-2 border-[var(--ui-bg-elevated)] bg-primary-500/10">
											<Icon name={methodIcon(p.method)} class="size-3 text-primary-500" />
										</div>
										<div class="space-y-0.5">
											<p class="text-xs font-bold capitalize leading-tight">
	...
	<section class="surface-card overflow-hidden">
		...
		<div class="-mx-3 overflow-x-auto p-3 sm:mx-0 sm:p-4">
			<div class="min-w-[540px] overflow-hidden rounded-xl border border-[var(--ui-border-muted)] sm:min-w-[620px]">
				<!-- Header row -->
				<div class="grid border-b border-[var(--ui-border-muted)] bg-[var(--ui-bg-muted)]" style="grid-template-columns: 1.5fr repeat(4, minmax(110px, 1fr))">
					<div class="p-3">
						<span class="text-xs font-bold tracking-wider uppercase text-[var(--ui-text-dimmed)]">Feature</span>
					</div>
					{#each displayPlans as code (code)}
						<div class="relative flex flex-col items-center justify-center gap-0.5 border-l border-[var(--ui-border-muted)] p-3 text-center {code === planCode && subscription ? 'bg-primary-500/10 ring-1 ring-primary-500/30' : ''}">
							{#if code === planCode && subscription}
								<span class="absolute -top-0.5 rounded-b-md bg-primary-500 px-2 py-0.5 text-xs font-bold tracking-wider uppercase text-white">Current</span>
							{/if}
							<span class="text-xs font-bold capitalize {code === planCode && subscription ? 'text-primary-600 dark:text-primary-400' : ''}">
								{PLANS[code].name}
							</span>
				...
				<div class="divide-y divide-[var(--ui-border-muted)]">
					{#each MODULE_LIST as mod (mod.key)}
						<div class="grid items-center hover:bg-[var(--ui-bg-accented)]/50" style="grid-template-columns: 1.5fr repeat(4, minmax(110px, 1fr))">
							<div class="flex items-center gap-2 p-3">
								<Icon name={mod.type === 'limit' ? 'lucide:settings-2' : 'lucide:check-square'} class="size-3.5 text-[var(--ui-text-dimmed)]" />
								<span class="text-xs font-medium text-[var(--ui-text-muted)]">{mod.label}</span>
							</div>
							{#each displayPlans as code (code)}
								<div class="flex items-center justify-center border-l border-[var(--ui-border-muted)] p-3 text-center {code === planCode && subscription ? 'bg-primary-500/5' : ''}">
									{#if mod.type === 'limit'}
										<span class="text-xs font-bold {PLANS[code].limits[mod.key] === 'unlimited' ? 'text-emerald-500' : 'text-[var(--ui-text-muted)]'}">
									...
									{:else}
										...
										{:else}
											<Icon name="lucide:minus-circle" class="size-4 text-[var(--ui-text-dimmed)] opacity-40" />
										{/if}
									{/if}
								</div>
							{/each}
...
<Dialog bind:open={showUpgradeModal} title="Upgrade plan" size="md">
	<div class="space-y-3">
		<div class="rounded-xl border border-primary-500/10 bg-primary-500/5 p-3">
			<p class="text-xs font-medium leading-relaxed text-primary-600 dark:text-primary-400">
				Choose a plan to upgrade to. You'll be charged a prorated amount for the remainder of this billing period.
			</p>
		</div>
		<div class="max-h-[380px] space-y-2 overflow-y-auto pr-1">
			{#each upgradePlans as code (code)}
				...
				<button
					...
					class="w-full rounded-xl border p-3.5 text-left transition-all {selectedPlan === code ? 'border-primary-500 bg-primary-500/10 ring-1 ring-primary-500/20' : 'border-[var(--ui-border)] hover:border-primary-500/30'}"
				>
					<div class="mb-2 flex items-start justify-between">
						<div class="flex items-center gap-2.5">
							<div class="grid size-8 place-items-center rounded-lg bg-gradient-to-br {plan.gradient} text-white">
					...
					<div class="mt-2.5 flex flex-wrap gap-x-3 gap-y-1 border-t border-[var(--ui-border-muted)] pt-2.5 text-xs text-[var(--ui-text-muted)]">
						...
						<span class="flex items-center gap-1">
							...
							Staff: <strong class="text-[var(--ui-text)]">{getPlanLimitText(code, 'staff')}</strong>
						</span>
						<span class="flex items-center gap-1">
	...
	{#snippet footer()}
		<Button color="neutral" variant="ghost" onclick={() => { showUpgradeModal = false; selectedPlan = null; }}>Cancel</Button>
		<Button color="primary" icon="lucide:arrow-up" disabled={!selectedPlan || payLoading} onclick={() => selectedPlan && changePlan(selectedPlan)}>
...
<Dialog bind:open={showDowngradeModal} title="Downgrade plan" size="md">
	<div class="space-y-3">
		<div class="flex items-start gap-2.5 rounded-xl border border-amber-200/40 bg-amber-50 p-3.5 dark:bg-amber-950/20">
			<Icon name="lucide:info" class="mt-0.5 size-4 shrink-0 text-amber-600 dark:text-amber-400" />
			<p class="text-xs font-medium leading-relaxed text-amber-700 dark:text-amber-400">
				Downgrading will reduce your limits at the end of the current billing period. Some features may become unavailable.
			</p>
		</div>
		<div class="max-h-[300px] space-y-2 overflow-y-auto">
			{#each downgradePlans as code (code)}
				...
				<button
					type="button"
					onclick={() => (selectedPlan = code)}
					class="w-full rounded-xl border p-3.5 text-left transition-all {selectedPlan === code ? 'border-amber-500 bg-amber-500/10 ring-1 ring-amber-500/20' : 'border-[var(--ui-border)] hover:border-amber-500/30'}"
				>
	...
	{#snippet footer()}
		<Button color="neutral" variant="ghost" onclick={() => { showDowngradeModal = false; selectedPlan = null; }}>Cancel</Button>
		<Button color="neutral" variant="solid" disabled={!selectedPlan || payLoading} onclick={() => selectedPlan && changePlan(selectedPlan)}>
...
<Dialog bind:open={showCancelModal} title="Cancel subscription" size="sm">
	<div class="space-y-3">
		<div class="flex items-center gap-2">
			...
			<h3 class="font-display text-[15px] font-semibold text-red-600 dark:text-red-400">Are you sure?</h3>
		</div>
		<p class="text-sm leading-relaxed text-[var(--ui-text-muted)]">
			Your subscription will remain active until the end of the current billing period, then switch to the Free plan.
		</p>
		<div class="rounded-xl border border-[var(--ui-border-muted)] bg-[var(--ui-bg-muted)] p-4">
	...
	{#snippet footer()}
		<Button color="neutral" variant="ghost" onclick={() => { showCancelModal = false; }}>Keep plan</Button>
		<Button color="error" variant="solid" icon="lucide:trash-2" onclick={cancelSubscription}>Confirm cancel</Button>
	{/snippet}
</Dialog>
...
<Dialog bind:open={showPayModal} title="Pay invoice" size="md">
	{#if payingInvoice}
		<div class="space-y-4">
			<!-- Invoice summary -->
			<div class="rounded-xl border border-[var(--ui-border-muted)] bg-[var(--ui-bg-muted)] p-4 text-center">
				<p class="mb-1 text-xs font-bold tracking-widest uppercase text-[var(--ui-text-dimmed)]">{payingInvoice.invoiceNumber}</p>
				<p class="text-3xl font-black">{formatAmount(payingInvoice.amount)}</p>
				<p class="mt-1.5 text-xs text-[var(--ui-text-muted)]">Due: {formatDate(payingInvoice.dueAt)}</p>
			</div>

			{#if activeLightningInvoice}
				<!-- Lightning invoice view -->
				<div class="space-y-4 text-center">
					<div class="relative flex flex-col items-center overflow-hidden rounded-2xl border border-amber-200/50 bg-amber-50 p-5 dark:border-amber-900/30 dark:bg-amber-950/20">
						<span class="absolute right-2.5 top-2.5 flex size-2">
							<span class="absolute inline-flex size-full animate-ping rounded-full bg-amber-400 opacity-75"></span>
							<span class="relative inline-flex size-2 rounded-full bg-amber-500"></span>
						</span>
						<div class="grid size-40 place-items-center rounded-xl border border-amber-200/50 bg-white p-3 dark:bg-gray-900 dark:border-amber-500/10">
							<Icon name="lucide:zap" class="size-14 text-amber-500" />
						</div>
						<div class="mt-4 w-full space-y-2">
							<p class="break-all rounded-xl border border-amber-200/30 bg-white p-3 text-center font-mono text-xs text-amber-800 dark:bg-black/35 dark:text-amber-400 select-all">
								{activeLightningInvoice}
							</p>
							<div class="flex gap-2">
								<Button size="sm" color="neutral" variant="subtle" block icon="lucide:copy" onclick={() => copyText(activeLightningInvoice!, 'Invoice')}>Copy</Button>
								<Button size="sm" color="neutral" variant="solid" block icon="lucide:wallet" href={'lightning:' + (activeLightningInvoice ?? '')}>Open wallet</Button>
							</div>
							<Button size="sm" color="primary" variant="soft" block icon="lucide:refresh-cw" onclick={() => {
								if (payingInvoice) {
					...
					<div class="flex items-center justify-center gap-2 rounded-lg border border-amber-500/10 bg-amber-500/5 py-2">
					...
					<div class="grid grid-cols-2 gap-3">
						{#each paymentMethodOptions as m (m.code)}
							<button
								...
								class="rounded-xl border p-3 text-center transition-all {selectedMethod === m.code ? 'border-primary-500 bg-primary-500/10 ring-1 ring-primary-500/20' : 'border-[var(--ui-border)] hover:border-primary-500/30'}"
							>
