<script lang="ts">
	...
	import { TYPE, statusColor, type Membership, type MembershipSubscription, type MembershipCheckIn, type MembershipSubscriptionStatus } from '$lib/domain';
	...
	function resetPlanForm() {
		mName = ''; mMode = 'normal'; mPrice = ''; mPeriod = 'monthly'; mDays = 30;
		mBenefits = []; mTrialDays = 0; mDescription = ''; mColor = '#6366F1'; mActive = true;
	}

	function openCreate(t: Tab) {
		dlgTab = t; editingId = null;
		resetPlanForm();
		subCust = ''; subPlan = ''; subStatus = 'active'; subExtendDays = 0;
		ciCust = ''; ciResult = 'allowed';
		open = true;
	}

	function openEditPlan(p: { id: string; data: Membership }) {
		dlgTab = 'plans'; editingId = p.id;
		mName = p.data.name ?? '';
	...
	function openEditSub(s: { id: string; data: MembershipSubscription }) {
		dlgTab = 'subscriptions'; editingId = s.id;
	...
	function addBenefit() { mBenefits = [...mBenefits, '']; }
	function removeBenefit(idx: number) { mBenefits = mBenefits.filter((_, i) => i !== idx); }
	function updateBenefit(idx: number, val: string) { mBenefits = mBenefits.map((b, i) => (i === idx ? val : b)); }
	...
	async function save() {
		if (dlgTab === 'plans') {
			...
			await glo.upsert<Membership>(TYPE.membership, payload, editingId ? { id: editingId } : undefined);
			...
			if (isEditing) {
				...
				await glo.upsert<MembershipSubscription>(TYPE.membershipSubscription, update as unknown as MembershipSubscription, { id: editingId! });
				toast.success('Subscription updated');
			} else {
				const plan = plans.find((p) => p.data.name === subPlan);
				const now = new Date();
				const end = new Date(now.getTime() + ((plan?.data.durationDays ?? 30) * 86_400_000));
				await glo.upsert<MembershipSubscription>(TYPE.membershipSubscription, {
					customerName: subCust.trim(),
			...
			await glo.upsert<MembershipCheckIn>(TYPE.membershipCheckIn, {
	...
	const modeBadgeColor = (mode?: string) => mode === 'gym' ? 'info' : mode === 'hybrid' ? 'warning' : 'primary';
</script>
...
<div class="space-y-4">
	...
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
		<div class="surface-card p-4">
			<p class="text-[10px] font-semibold uppercase tracking-wider text-[var(--ui-text-dimmed)]">Total Plans</p>
			<p class="font-display text-2xl font-bold tabular-nums">{plans.length}</p>
		</div>
		<div class="surface-card p-4">
			<p class="text-[10px] font-semibold uppercase tracking-wider text-[var(--ui-text-dimmed)]">Active</p>
			<p class="font-display text-2xl font-bold tabular-nums text-emerald-500">{plans.filter((p) => (p.data.active ?? p.data.status === 'active')).length}</p>
		</div>
		<div class="surface-card p-4">
			<p class="text-[10px] font-semibold uppercase tracking-wider text-[var(--ui-text-dimmed)]">Subscriptions</p>
			<p class="font-display text-2xl font-bold tabular-nums text-blue-500">{subs.filter((s) => s.data.status === 'active').length}</p>
		</div>
		<div class="surface-card p-4">
			<p class="text-[10px] font-semibold uppercase tracking-wider text-[var(--ui-text-dimmed)]">Check-ins Today</p>
			<p class="font-display text-2xl font-bold tabular-nums">{checkins.filter((c) => { const d = new Date(c.data.occurredAt); const t = new Date(); return d.getDate() === t.getDate() && d.getMonth() === t.getMonth() && d.getFullYear() === t.getFullYear(); }).length}</p>
		</div>
	</div>
		...
		{:else}
			<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
				{#each planCtrl.pagedList as p (p.id)}
					<div class="accent-bar surface-card p-5" style="--accent:{p.data.color ?? 'var(--ui-color-primary-500)'};">
						<div class="flex items-start justify-between">
							<div class="grid size-10 place-items-center rounded-xl bg-primary-500/10 text-primary-600 dark:text-primary-400" style="background-color:{(p.data.color ?? '#6366F1') === '#6366F1' ? '' : (p.data.color ?? '#6366F1') + '1a'}; color:{p.data.color ?? '#6366F1'};">
								<Icon name={p.data.mode === 'gym' ? 'lucide:dumbbell' : 'lucide:crown'} class="size-5" />
							</div>
							<div class="flex items-center gap-1">
								<Badge color={p.data.active ?? p.data.status === 'active' ? 'success' : 'neutral'}>{p.data.active ?? p.data.status === 'active' ? 'active' : 'inactive'}</Badge>
						...
						<div class="font-display text-3xl font-bold tabular-nums">{formatMoney(p.data.price, p.data.currency || currency)}<span class="text-[12px] font-medium text-[var(--ui-text-dimmed)]">/{p.data.period}</span></div>
						<div class="mt-1 text-[11.5px] text-[var(--ui-text-muted)]">{p.data.durationDays ?? '∞'} days duration{#if (p.data as any).trialDays} · {(p.data as any).trialDays}d trial{/if}</div>

						{#if p.data.description}
							<p class="mt-2 text-[12px] text-[var(--ui-text-muted)] line-clamp-2">{p.data.description}</p>
						{/if}

						{#if p.data.benefits && p.data.benefits.length > 0}
							<div class="mt-2 flex flex-wrap gap-1">
								{#each p.data.benefits.slice(0, 4) as benefit (benefit)}
									<span class="rounded-full bg-[var(--ui-bg-accented)] px-2 py-0.5 text-[10px] font-medium text-[var(--ui-text-muted)]">{benefit}</span>
								{/each}
								{#if p.data.benefits.length > 4}
									<span class="rounded-full bg-[var(--ui-bg-accented)] px-2 py-0.5 text-[10px] font-medium text-[var(--ui-text-dimmed)]">+{p.data.benefits.length - 4}</span>
								{/if}
							</div>
						{/if}

						<div class="mt-3 flex items-center justify-between border-t border-[var(--ui-border-muted)] pt-3">
							<label class="flex items-center gap-1.5 text-[11px] text-[var(--ui-text-muted)]">
								<Switch checked={p.data.active ?? p.data.status === 'active'} onCheckedChange={async (v) => { await glo.upsert<Membership>(TYPE.membership, { ...p.data, active: v, status: v ? 'active' : 'inactive' }, { id: p.id }); toast.info(v ? 'Activated' : 'Deactivated'); }} />
								Active
							</label>
							<div class="flex items-center gap-1">
								<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:pencil" onclick={() => openEditPlan(p)} />
								<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:trash-2" onclick={() => { glo.remove(TYPE.membership, p.id); toast.info('Removed'); }} />
							</div>
						</div>
					</div>
				{/each}
	...
	{:else if tab === 'subscriptions'}
		...
		{:else}
			<div class="data-panel">
				<table class="table-surface w-full text-left"><thead><tr><th class="px-5 py-2.5">Customer</th><th class="px-5 py-2.5">Plan</th><th class="px-5 py-2.5">Status</th><th class="px-5 py-2.5 text-right">Ends</th><th class="px-5 py-2.5 text-right">Actions</th></tr></thead>
					<tbody class="divide-y divide-[var(--ui-border-muted)] text-[13px]">
						{#each subCtrl.pagedList as s (s.id)}<tr class="hover:bg-[var(--ui-bg-accented)]/40">
							...
							<td class="px-5 py-3 text-right text-[12px] text-[var(--ui-text-dimmed)]">{s.data.endsAt ? relativeTime(s.data.endsAt) : '—'}</td>
							<td class="px-5 py-3">
								<div class="flex items-center justify-end gap-1">
									<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:pencil" onclick={() => openEditSub(s)} />
									<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:trash-2" onclick={() => { glo.remove(TYPE.membershipSubscription, s.id); toast.info('Removed'); }} />
								</div>
							</td>
						</tr>{/each}
					</tbody>
...
<Dialog bind:open={open} size="lg" title={dlgTab === 'plans' ? (isEditing ? 'Edit plan' : 'New plan') : dlgTab === 'subscriptions' ? (isEditing ? 'Edit subscription' : 'New subscription') : 'New check-in'}>
	{#if dlgTab === 'plans'}
		<div class="space-y-4">
			<!-- Name + Color -->
			<div class="flex items-end gap-3">
				<div class="grid size-12 shrink-0 place-items-center rounded-xl border-2 border-dashed" style="background-color:{mColor}1a; border-color:{mColor}; color:{mColor}">
					<Icon name={mMode === 'gym' ? 'lucide:dumbbell' : 'lucide:crown'} class="size-5" />
				</div>
				<label class="flex-1">
					<span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Plan name</span>
					<Input bind:value={mName} class="w-full" placeholder="Gold Member" />
				</label>
				<label>
					<span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Color</span>
					<input type="color" bind:value={mColor} class="h-9.5 w-14 cursor-pointer rounded-lg border border-[var(--ui-border)] bg-[var(--ui-bg-muted)] p-1" />
				</label>
			</div>
			...
			{#if mDescription !== undefined}
				<label class="block">
					...
					<Input textarea bind:value={mDescription} rows={2} class="w-full" placeholder="Brief description of this plan..." />
				</label>
			{/if}
			...
			<div>
				...
				<div class="grid grid-cols-3 gap-2">
					{#each [{ v: 'normal', l: 'Normal', i: 'lucide:star' }, { v: 'gym', l: 'Gym', i: 'lucide:dumbbell' }, { v: 'hybrid', l: 'Hybrid', i: 'lucide:layers' }] as m (m.v)}
						<button type="button" onclick={() => (mMode = m.v as Membership['mode'])} class="flex flex-col items-center gap-1 rounded-lg border-2 px-2 py-2.5 text-[11px] font-medium transition-colors {mMode === m.v ? 'border-primary-500 bg-primary-500/10 text-primary-600 dark:text-primary-400' : 'border-[var(--ui-border)] hover:border-[var(--ui-border-accented)]'}">
			...
			<div class="grid grid-cols-2 gap-3">
				...
				<label class="block"><span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Period</span><Select bind:value={mPeriod} options={[{ value: 'daily', label: 'Daily' }, { value: 'weekly', label: 'Weekly' }, { value: 'monthly', label: 'Monthly' }, { value: 'quarterly', label: 'Quarterly' }, { value: 'yearly', label: 'Yearly' }, { value: 'one_time', label: 'One Time' }]} class="w-full" /></label>
			</div>
			<div class="grid grid-cols-2 gap-3">
				...
				<label class="block"><span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Trial period (days)</span><Input bind:value={mTrialDays} type="number" min="0" class="w-full" placeholder="0" /></label>
			</div>

			<!-- Benefits -->
			<div>
				<span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Benefits</span>
				<div class="space-y-2">
					{#each mBenefits as benefit, i (i)}
						<div class="flex gap-2">
							<Input value={benefit} oninput={(e) => updateBenefit(i, e.currentTarget.value)} class="flex-1" placeholder="e.g. Free coffee" />
							<Button color="neutral" variant="ghost" size="icon-sm" icon="lucide:x" onclick={() => removeBenefit(i)} />
						</div>
					{/each}
					<button type="button" onclick={addBenefit} class="inline-flex items-center gap-1 text-[12px] font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400">
	...
	{:else if dlgTab === 'subscriptions'}
		<div class="space-y-3">
			...
			<label class="block">
				...
				<datalist id="planslist">{#each plans as p (p.id)}<option value={p.data.name}></option>{/each}</datalist>
			</label>
			{#if isEditing}
				<label class="block"><span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Status</span><Select bind:value={subStatus} options={[{ value: 'active', label: 'Active' }, { value: 'suspended', label: 'Suspended' }, { value: 'frozen', label: 'Frozen' }, { value: 'cancelled', label: 'Cancelled' }, { value: 'expired', label: 'Expired' }]} class="w-full" /></label>
				<label class="block"><span class="mb-1.5 block text-[12px] font-semibold text-[var(--ui-text-muted)]">Extend duration (days)</span><Input bind:value={subExtendDays} type="number" min="0" class="w-full" placeholder="0 = no change" /></label>
			{/if}
		</div>
	{:else}
		<div class="space-y-3">
	...
	{/if}
	{#snippet footer()}<Button color="neutral" variant="ghost" onclick={() => { open = false; editingId = null; }}>Cancel</Button><Button color="primary" icon="lucide:check" onclick={save}>{isEditing ? 'Update' : 'Save'}</Button>{/snippet}
</Dialog>
