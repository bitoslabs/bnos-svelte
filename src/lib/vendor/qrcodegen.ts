...
/**
 * QR Code generator library (TypeScript) — vendored from Project Nayuki (MIT).
 * https://www.nayuki.io/page/qr-code-generator-library
 *
 * This is the canonical, battle-tested implementation (the same one bundled by
 * naive-ui / vue-qrcode). Vendored here so payment QR codes render offline with
 * zero runtime dependencies. License: MIT — see header above. Unmodified logic.
 */
'use strict';
...
(function (qrcodegen: any) {
	/* ---- QR Code symbol class ---- */
	class QrCode {
		static encodeText(text: string, ecl: any): QrCode {
			const segs = qrcodegen.QrSegment.makeSegments(text);
			return QrCode.encodeSegments(segs, ecl);
		}

		static encodeBinary(data: number[], ecl: any): QrCode {
		...
		static getNumDataCodewords(ver: number, ecl: any): number {
			return (
		...
		finderPenaltyCountPatterns(runHistory: number[]): number {
		...
		finderPenaltyAddHistory(currentRunLength: number, runHistory: number[]): void {
			if (runHistory[0] === 0) {
				currentRunLength += this.size;
			}
			runHistory.pop();
	...
	class QrSegment {
		static makeBytes(data: number[]): QrSegment {
			...
			return new QrSegment(qrcodegen.QrSegment.Mode.BYTE, data.length, bb);
		}
		...
		static makeNumeric(digits: string): QrSegment {
			...
			return new QrSegment(qrcodegen.QrSegment.Mode.NUMERIC, digits.length, bb);
		}
		...
		static makeAlphanumeric(text: string): QrSegment {
			...
			return new QrSegment(qrcodegen.QrSegment.Mode.ALPHANUMERIC, text.length, bb);
		}
		...
		static makeEci(assignVal: number): QrSegment {
			...
			return new QrSegment(qrcodegen.QrSegment.Mode.ECI, 0, bb);
		}
		...
		constructor(mode: any, numChars: number, bitData: number[]) {
			this.mode = mode;
...
(function (qrcodegen: any) {
	...
	(function (QrCode: any) {
		class Ecc {
			constructor(public ordinal: number, public formatBits: number) {}
...
(function (qrcodegen: any) {
	let QrSegment = qrcodegen.QrSegment;
	(function (QrSegment: any) {
		class Mode {
			constructor(public modeBits: number, public numBitsCharCount: number[]) {}
