 ...
 * (`'none'`), uploads fall back to the server endpoint `/api/media/upload`,
 * which signs them server-side using `BNOS_CLOUDINARY_*` env vars.
 *
...
export function providerLabel(id: UploadedMediaProviderId | 'none'): string {
	if (id === 'none') return 'Server fallback';
	if (id === 'server') return 'BNOS uploads';
...
class MediaStore {
	...
	/** True when uploads will route through the server fallback. */
	...
	upload = async (
		...
		const owner = options.owner ?? tenant.state.organizationId ?? '';
		if (id === 'none') {
			return uploadViaServer(file, { ...options, owner });
		}
